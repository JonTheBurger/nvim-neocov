...
function tree:reduce(node, key, parent)
  for k, v in pairs(node) do
    if type(v) == "table" then self:reduce(v, k, node) end
  end
  if #node == 1 and not self.options.noreduce[key] and node._attr == nil then parent[key] = node[1] end
end
...
function tree:starttag(tag)
  local node = {}
  if self.parseAttributes == true then node._attr = tag.attrs end
...
function tree:endtag(tag, s)
  ...
  if not prev[tag.name] then error("XML Error - Unmatched Tag [" .. s .. ":" .. tag.name .. "]\n") end
