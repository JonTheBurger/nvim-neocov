...
function dom:starttag(tag)
  ...
  if not self.root then self.root = node end
...
function dom:endtag(tag)
  ...
  if tag.name ~= prev._name then error("XML Error - Unmatched Tag [" .. s .. ":" .. tag.name .. "]\n") end
...
function dom:decl(tag)
  if self.options.declNode then self.decl = { _type = "DECL", _name = tag.name, _attr = tag.attrs } end
end
...
function dom:dtd(tag)
  if self.options.dtdNode then self.dtd = { _type = "DTD", _name = tag.name, _text = tag.value } end
end
...
local function toXmlStr(node, indentLevel)
  if not node then return "BUG:node==nil" end
  if not node._type then return "BUG:node._type==nil" end
  ...
  elseif node._type == "ELEMENT" then
    ...
    if not node._children or #node._children == 0 then return s .. "/>\n" end
...
function dom:toXml(node) return toXmlStr(node, -4) end
