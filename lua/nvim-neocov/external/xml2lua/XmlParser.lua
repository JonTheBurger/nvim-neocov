...
local function decimalToHtmlChar(code)
  ...
  if num >= 0 and num < 256 then return string.char(num) end
...
--               local handler = require("nvim-neocov.external.xml2lua.xmlhandler/tree").
...
local function err(self, errMsg, pos)
  if self.options.errorHandler then self.options.errorHandler(errMsg, pos) end
end
...
local function parseXmlDeclaration(self, xml, f)
  ...
  if not f.match then err(self, self._errstr.declErr, f.pos) end
  ...
  if tag.attrs and tag.attrs.version == nil then err(self, self._errstr.declAttrErr, f.pos) end

  if fexists(self.handler, "decl") then self.handler:decl(tag, f.match, f.endMatch) end
...
local function parseXmlProcessingInstruction(self, xml, f)
  ...
  f.match, f.endMatch, f.text = string.find(xml, self._PI, f.pos)
  if not f.match then err(self, self._errstr.piErr, f.pos) end
...
local function parseComment(self, xml, f)
  ...
  if not f.match then err(self, self._errstr.commentErr, f.pos) end
...
local function _parseDtd(self, xml, pos)
  ...
  for _, dtd in pairs(dtdPatterns) do
    ...
    if m then return m, e, { _root = r, _type = t, _name = n, _uri = u, _internal = i } end
  end
...
local function parseDtd(self, xml, f)
  ...
  if not f.match then err(self, self._errstr.dtdErr, f.pos) end
...
local function parseCdata(self, xml, f)
  ...
  if not f.match then err(self, self._errstr.cdataErr, f.pos) end

  if fexists(self.handler, "cdata") then self.handler:cdata(f.text, nil, f.match, f.endMatch) end
end
...
local function parseNormalTag(self, xml, f)
  ...
  while 1 do
    ...
    if f.errEnd == nil then
      ...
      if f.errEnd == nil then break end
    end
    ...
    if not f.match then err(self, self._errstr.xmlErr, f.pos) end
    f.endMatch = f.extEnd
  end
  ...
  if f.endt1 == "/" then
    ...
    if fexists(self.handler, "starttag") then self.handler:starttag(tag, f.match, f.endMatch) end

    -- Self-Closing Tag
    if f.endt2 == "/" then
      table.remove(self._stack)
      if fexists(self.handler, "endtag") then self.handler:endtag(tag, f.match, f.endMatch) end
    end
  end
...
function XmlParser:parse(xml, parseAttributes)
  ...
  while f.match do
    if not getNextTag(self, xml, f) then break end
    ...
    f.text = parseEntities(self, stripWS(self, f.text))
    if f.text ~= "" and fexists(self.handler, "text") then self.handler:text(f.text, nil, f.match, f.endText) end
